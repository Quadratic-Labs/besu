...
public final class UInt256 {
  ...
  // Broadcast lookup table for SIMD addition carry propagation
  private static final byte[] BROADCAST_LOOKUP = new byte[] {
  ...
  /**
   * SIMD-optimized addition using broadcast lookup table for carry propagation.
   *
   * <p>This method performs addition by treating the 256-bit integer as 4 64-bit chunks
   * and uses a lookup table to efficiently handle carry propagation across chunks.
   *
   * @param other The integer to add to this.
   * @return A new UInt256 representing this + other (wrapping on overflow).
   */
  public UInt256 addSIMD(final UInt256 other) {
    int[] result = new int[N_LIMBS];

    // Treat the 8 32-bit limbs as 4 64-bit chunks for SIMD-style processing
    // Each chunk is represented by two consecutive limbs (little-endian)
    long[] thisChunks = new long[4];
    long[] otherChunks = new long[4];
    long[] sumChunks = new long[4];

    // Convert limbs to 64-bit chunks
    for (int i = 0; i < 4; i++) {
      thisChunks[i] = ((this.limbs[2 * i] & MASK_L) | ((this.limbs[2 * i + 1] & MASK_L) << 32));
      otherChunks[i] = ((other.limbs[2 * i] & MASK_L) | ((other.limbs[2 * i + 1] & MASK_L) << 32));
    }

    // Perform initial addition for each 64-bit chunk
    for (int i = 0; i < 4; i++) {
      sumChunks[i] = thisChunks[i] + otherChunks[i];
    }

    // Detect carries using comparison (carry occurs when sum < either operand)
    int carryMask = 0;
    for (int i = 0; i < 4; i++) {
      if (Long.compareUnsigned(sumChunks[i], thisChunks[i]) < 0) {
        carryMask |= (1 << i);
      }
    }

    // Use broadcast lookup table to propagate carries
    int lookupOffset = carryMask * 32; // Each pattern takes 32 bytes (4 longs)

    // Apply carry propagation from lookup table
    for (int i = 0; i < 4; i++) {
      long carryValue = BROADCAST_LOOKUP[lookupOffset + i * 8] & 0xFFL;
      sumChunks[i] += carryValue;
    }

    // Convert 64-bit chunks back to 32-bit limbs
    for (int i = 0; i < 4; i++) {
      result[2 * i] = (int) sumChunks[i];
      result[2 * i + 1] = (int) (sumChunks[i] >>> 32);
    }
